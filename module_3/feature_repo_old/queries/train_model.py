...
class CreditXGBClassifier:
    ...
    def __init__(self, fs, data_fetcher):
        ...
        print(f"Columns int training df: {self.training_df.columns}")
        print(f"Columns to drop: {self._data_cls.columns_to_drop}")
    ...
    def train(self) -> None:
        ...
        # print("Train df's head: ", X_train.head())
    ...
    def predict(self, request):
        ...
        #print(f"zipcode features: {zipcode_features}")

        # Join features to request features
        features = request.copy()
        features.update(zipcode_features)
        features_df = pd.DataFrame.from_dict(features)
        #print(f"Features df: {features_df}")

        # Sort columns
        features_df = features_df.reindex(sorted(features_df.columns), axis=1)
        #print(f"Reindexed features df: {features_df}")

        # Apply ordinal encoding to categorical features
        self._apply_ordinal_encoding(features_df)
        #print(f"Ordinal encoded features df: {features_df}")

        # Make prediction
        data = xgb.DMatrix(features_df, enable_categorical=True)
